...
- **In-Transit Encryption:** All communication between mobile browsers, gate scanners, and our serverless API endpoints is encrypted using industry-standard `TLS 1.3 / HTTPS`.
- **At-Rest Storage:** User records, event data, and cryptographic tickets are stored securely inside **Google Cloud Firestore (`Firebase Admin SDK`)** across multi-region databases with enterprise IAM access controls.
- **KYC & Receipt Storage:** Sensitive CNIC images and bank receipts are stored in **Firebase Cloud Storage** protected by strict signed-URL token boundaries.
